"""
X-vector verification model for downstream audio-visual tasks
Modified from https://github.com/s3prl/s3prl/blob/main/s3prl/downstream/sv_voxceleb1/model.py
"""
...
class ASP(nn.Module):
    ...
    def forward(self, feature_BxTxH, att_mask_BxT):
        ...
        variance = torch.sqrt(
            torch.sum(att_w * feature_BxTxH * feature_BxTxH, dim=1)
            - sap_vec**2
            + 1e-8
        )
...
class Model(nn.Module):
    def __init__(
        ...
        utterance_input_dim = decide_utter_input_dim(
            agg_module_name=agg_module_name, agg_dim=agg_dim, input_dim=input_dim
        )

        # after extract utterance level vector, put it to utterance extractor (XVector Architecture)
        utterance_extractor_config = {
            "input_dim": utterance_input_dim,
            "out_dim": input_dim,
        }
        self.utterancelevel_feature_extractor = eval(utterance_module_name)(
            **utterance_extractor_config
        )

    def forward(self, features_BxTxH, att_mask_BxT):
        features_BxTxH = self.framelevel_feature_extractor(
            features_BxTxH, att_mask_BxT[:, None, None]
        )
    ...
    def inference(self, features_BxTxH, att_mask_BxT):
        ...
        utterance_vector = self.agg_method(features_BxTxH, att_mask_BxT)
        utterance_vector = self.utterancelevel_feature_extractor.inference(
            utterance_vector
        )
...
class AMSoftmaxLoss(nn.Module):
    def __init__(self, hidden_dim, speaker_num, s=30.0, m=0.4, **kwargs):
        ...
        self.W = torch.nn.Parameter(
            torch.randn(hidden_dim, speaker_num), requires_grad=True
        )
...
class SoftmaxLoss(nn.Module):
    ...
    def forward(self, x_BxH, labels_B):
        ...
        return loss, logits_BxSpn


class AAMSoftmaxLoss(nn.Module):
    def __init__(
        self, hidden_dim, speaker_num, s=15, m=0.3, easy_margin=False, **kwargs
    ):
        ...
        self.weight = torch.nn.Parameter(
            torch.FloatTensor(speaker_num, hidden_dim), requires_grad=True
        )
