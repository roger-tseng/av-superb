...
train_group = ["g10", "g11", "g12", "g13", "g14", "g15", "g16", "g17", "g18", "g19", "g20", "g21", "g22", "g23", "g24", "g25"]
dev_group = ["g08", "g09"]
...
for typeName in os.listdir(f"{origin_base_path}"):
    ...
    for aviFile in os.listdir(f"{origin_base_path}/{typeName}"):
        ...
        if group in train_group:
            output = ffmpeg.output(audio, video, f"{preprocessed_base_path}/train/{group}_{clip}_{ind}.mp4")
        elif group in dev_group:
            output = ffmpeg.output(audio, video, f"{preprocessed_base_path}/dev/{group}_{clip}_{ind}.mp4")
        elif group in test_group:
            output = ffmpeg.output(audio, video, f"{preprocessed_base_path}/test/{group}_{clip}_{ind}.mp4")
