...
class UniformDrop(object):
    ...
    def __call__(self, clip):
        if self.num_frames is not None:
            if len(clip) <= self.num_frames:
                return clip
            idx = np.linspace(0, len(clip) - 1, self.num_frames, endpoint=True).astype(
                int
            )
            return [clip[i] for i in idx]
...
class RandomResizedCrop(object):
    ...
    def __call__(self, clip):
        ...
        return [
            vF.resized_crop(img, i, j, h, w, self.size, self.interpolation)
            for img in clip
        ]
...
class TemporalJitter(object):
    ...
    def __init__(self, n_frames, time_scale=(1.0, 1.0)):
    ...
    def __call__(self, clip):
        ...
        rate = (
            self.time_scale[0]
            if self.time_scale[0] == self.time_scale[1]
            else random.uniform(
                self.time_scale[0],
                min(self.time_scale[1], float(len(clip)) / float(self.n_frames)),
            )
        )
...
class TemporalCenterCrop(object):
    ...
    def __init__(self, n_frames, time_scale=1.0):
        self.n_frames = n_frames
        self.time_scale = time_scale
