...
from torchvision.transforms._transforms_video import NormalizeVideo, ToTensorVideo
...
class UpstreamExpert(nn.Module):
    def __init__(self, ckpt: str = None, model_config: str = None, **kwargs):
        ...
        self.ft = (
            True if any("blocks_av.2" in key for key in ckpt["model"].keys()) else False
        )
    ...
    def preprocess_audio(
        self, audio, audio_sample_rate, fbank_mean=None, fbank_std=None
    ):
    ...
    def forward(
        ...
        for blk in self.model.blocks:
            ...
            audio_pooled_feats.append(
                x[:, 1:, :].mean(dim=1, keepdim=True)
            )  # drop CLS token and pool
            x = blk(x)
        ...
        for blk in self.model.blocks_v:
            ...
            video_pooled_feats.append(
                v[:, 1:, :].mean(dim=1, keepdim=True)
            )  # drop CLS token and pool
            v = blk(v)
        ...
        if self.model.av_fusion:
            ...
            if self.ft:
                fusion_pooled_feats.append(
                    self.model.fc_norm_av(torch.cat((x, v), dim=2))
                )
            else:
                fusion_pooled_feats.append(torch.cat((x, v), dim=2))
