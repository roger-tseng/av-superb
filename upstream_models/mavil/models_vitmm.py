...
class VisionTransformerMM(timm.models.vision_transformer.VisionTransformer):
    ...
    """
    def __init__(self, global_pool=False, mask_2d=True, av_fusion=False, n_frm=8, depth_av=0, pos_train=False, roll_mag_aug=False, dataset="audioset", **kwargs):
        ...
        self.patch_embed_v = PatchEmbed3D_new(video_size=(n_frm, 224, 224), 
        ...
        self.pos_embed_v = nn.Parameter(torch.zeros(1, self.num_patches_v + 1, embed_dim), requires_grad=pos_train)  
        self.blocks_v = nn.ModuleList([Block(embed_dim, num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer) for i in range(depth)])
    ...
    def forward(self, source, mask_t_prob=0.0, mask_f_prob=0.0, mask_v_prob=0.0):
        ...
        if mask_t_prob > 0.0 or mask_f_prob > 0.0 or mask_v_prob > 0.0:
            x, v = self.forward_features_av_mask(x, v, mask_t_prob=mask_t_prob, mask_f_prob=mask_f_prob, mask_v_prob=mask_v_prob)
        else:
