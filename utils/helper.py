...
def parse_prune_heads(config):
    if (
        "prune_headids" in config["transformer"]
        and config["transformer"]["prune_headids"] != "None"
    ):
        heads_int = []
...
##########################
def get_transformer_tester(
    from_path="result/result_transformer/libri_sd1337_fmllrBase960-F-N-K-RA/model-1000000.ckpt",
